...
class RL_kdog_futures_custom_stoploss(IStrategy):
    """
Here be stonks
1. freqtrade hyperopt --hyperopt-loss SharpeHyperOptLoss --strategy RL_kdog_futures_custom_stoploss --freqaimodel ReinforcementLearner --spaces sell roi --timerange "$(date --date='-1 month' '+%Y%m%d')"-"$(date '+%Y%m%d')" -e 1000
2. freqtrade trade --logfile ./logs --freqaimodel ReinforcementLearner_multiproc --strategy RL_kdog_futures
    ...
    time_stop_1 = DecimalParameter(-0.15, -0.05, default=-0.05, decimals=2, space='sell', optimize=True)
    time_stop_2 = DecimalParameter(-0.25, -0.15, default=-0.15, decimals=2, space='sell', optimize=True)
    time_stop_3 = DecimalParameter(-0.35, -0.25, default=-0.25, decimals=2, space='sell', optimize=True)
    time_stop_4 = DecimalParameter(-0.45 -0.35, default=-0.35, decimals=2, space='sell', optimize=True)
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        if current_time - timedelta(minutes=60) > trade.open_date:
            return self.time_stop_1.value
        elif current_time - timedelta(minutes=30) > trade.open_date:
            return self.time_stop_2.value
        elif current_time - timedelta(minutes=15) > trade.open_date:
            return self.time_stop_3.value
        elif current_time - timedelta(minutes=0) > trade.open_date:
            return self.time_stop_4.value
        return 1
